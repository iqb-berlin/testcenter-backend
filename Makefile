...
composer-install:
	...
	docker run -v $(CURDIR)/composer.json:/composer.json -v $(CURDIR)/composer.lock:/composer.lock -v $(CURDIR)/vendor:/vendor testcenter-backend-composer composer install --no-interaction --no-ansi

composer-update:
	docker build -f docker/Dockerfile --target backend-composer -t testcenter-backend-composer:latest .
	docker run -v $(CURDIR)/auth.json:/auth.json  -v $(CURDIR)/composer.json:/composer.json -v $(CURDIR)/composer.lock:/composer.lock -v $(CURDIR)/vendor:/vendor testcenter-backend-composer composer update --no-interaction --no-ansi
